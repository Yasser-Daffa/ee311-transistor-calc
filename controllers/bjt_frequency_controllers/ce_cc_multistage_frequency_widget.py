...
UI_PATH = os.path.join(
    ...
    "ui",
    "frequency",
    "ce_cc_multistage_frequency.ui",
)


class CECCMultistageFrequencyWidget(QWidget):
    def __init__(self):
        super().__init__()
        loadUi(UI_PATH, self)
    ...
    def connect_signals(self):
        ...
        self.buttonOpen.clicked.connect(self.open_ce_cc_design_window)
    ...
    def open_ce_cc_design_window(self):
        self.ce_design_window = MultistageChoicesWidget()
        self.ce_design_window.buttonCECC.clicked.emit()
        self.ce_design_window.buttonCECC.setChecked(True)
        self.ce_design_window.setWindowTitle("Multistage Design/Analysis Calculator")
        self.ce_design_window.resize(1100, 750)
        self.ce_design_window.show()

    def set_low_status(self, text):
        # In your UI, low status label is named labelMode.
        if hasattr(self, "labelMode"):
            self.labelMode.setText(text)

    def set_high_status(self, text):
        # In your UI, high status label is currently named labelMode_2.
        # This also supports labelHighMode if you rename it later.
        if hasattr(self, "labelHighMode"):
            self.labelHighMode.setText(text)
        elif hasattr(self, "labelMode_2"):
            self.labelMode_2.setText(text)

    def open_design_window_placeholder(self):
        """
        Connect this later to your CE-CC multistage design/analysis page.
        For now it only tells the user that the external design link is not wired.
        """
        self.set_low_status("Design page link is not connected yet")
        self.set_high_status("Design page link is not connected yet")
    ...
    def update_rb1_input_lock(self):
        """
        Prevent invalid RB1 input combinations.
    ...
    def read_inputs(self):
        ...
        return {
            ...
            # CC stage 2
            "rb2": self.read_kohm(self.lineRB2),
    ...
    def calculate_low(self, v):
        ...
        # Stage 2 CC helper values
        if re2 is not None and rl is not None:
            result["REL2"] = parallel(re2, rl)

        if rpi2 is not None and beta is not None and result["REL2"] is not None:
            result["RXX2"] = rpi2 + (beta + 1) * result["REL2"]
        ...
        # R22 and f2: interstage capacitor C2
        ...
        # R33 and f3: CC output capacitor C3
        if (
            rl is not None
            and re2 is not None
            ...
            result["R33"] = rl + parallel(
                re2,
                (rpi2 + result["RSB2"]) / (beta + 1),
            )

            if c3 is not None:
                result["f3"] = cutoff_frequency(result["R33"], c3)

        # REE and fE: CE emitter bypass capacitor
        if (
            re1 is not None
            and rx1 is not None
    ...
    def calculate_high(self, v, low):
        ...
        # CE stage 1 loaded collector resistance
        # RCL1 = RC1 || Ri2
        ...
        # CC stage 2 Rπ
        # Rπ2 = rπ2 || [RSB2 + (β + 1)REL2]
        if (
            rpi2 is not None
            and beta is not None
            and result["RSB2"] is not None
            and result["REL2"] is not None
        ):
            result["Rpi2"] = parallel(
                rpi2,
                result["RSB2"] + (beta + 1) * result["REL2"],
            )

        # CC stage 2 Rµ
        # Rµ2 = RSB2 || RXX2
        if result["RSB2"] is not None and result["RXX2"] is not None:
            result["Rmu2"] = parallel(result["RSB2"], result["RXX2"])
    ...
    # ------------------------------------------------------------
    # Output helpers matching your uploaded UI object names
    # ------------------------------------------------------------
    ...
    def show_high_result(self, r):
        # Your uploaded UI names Rπ labels as labelRpi1_2 and labelRpi2_2.
...
if __name__ == "__main__":
    ...
    w = CECCMultistageFrequencyWidget()
    w.show()
    sys.exit(app.exec())
