...
UI_PATH = os.path.join(
    ...
    "ui",
    "frequency",
    "cc_ce_multistage_frequency.ui",
)


class CCCEMultistageFrequencyWidget(QWidget):
    def __init__(self):
        super().__init__()
        loadUi(UI_PATH, self)
    ...
    def connect_signals(self):
        inputs = self.all_input_lines()

        for line in inputs:
            line.textChanged.connect(self.calculate)

        self.lineR1.textChanged.connect(self.update_rb2_input_lock)
        self.lineR2.textChanged.connect(self.update_rb2_input_lock)
        ...
        if hasattr(self, "buttonOpen"):
            self.buttonOpen.clicked.connect(self.open_cc_ce_design_window)

    # ------------------------------------------------------------
    # UI helpers
    # ------------------------------------------------------------

    def open_cc_ce_design_window(self):
        self.design_window = MultistageChoicesWidget()
        self.design_window.buttonCCCE.clicked.emit()
        self.design_window.buttonCCCE.setChecked(True)
    ...
    def update_rb2_input_lock(self):
        """
        Prevent invalid RB2 input combinations for the CE stage.
        ...
        if has_r1_or_r2 and not has_rb2:
            self.lineRB2.setEnabled(False)
            self.lineRB2.setToolTip(
                "Disabled because R1 or R2 is entered. Clear R1/R2 to use RB2 direct."
            )
        ...
        if has_rb2 and not has_r1_or_r2:
            self.lineR1.setEnabled(False)
            self.lineR2.setEnabled(False)
            self.lineR1.setToolTip(
                "Disabled because RB2 direct is entered. Clear RB2 to use R1/R2."
            )
            self.lineR2.setToolTip(
                "Disabled because RB2 direct is entered. Clear RB2 to use R1/R2."
            )
        ...
        if not has_r1_or_r2 and not has_rb2:
            self.lineR1.setEnabled(True)
            self.lineR2.setEnabled(True)
            self.lineRB2.setEnabled(True)
    ...
    def get_rb2(self, r1, r2, rb2_direct):
    ...
    def read_inputs(self):
        ...
        return {
            ...
            # CC stage 1
            "rb1": self.read_kohm(self.lineRB1),
            ...
            "r1": r1,
            "r2": r2,
            "rb2": self.get_rb2(r1, r2, rb2_direct),
    ...
    def calculate_low(self, v):
        ...
        # CC stage 1 helper:
        # REL1 = RE1 || Ri2
        if re1 is not None and result["Ri2"] is not None:
            result["REL1"] = parallel(re1, result["Ri2"])
        ...
        # RXX1 = rπ1 + (β + 1)REL1
        if rpi1 is not None and beta is not None and result["REL1"] is not None:
            result["RXX1"] = rpi1 + (beta + 1) * result["REL1"]

        # CC stage 1 output resistance:
        # Rout1 = RE1 || [(rπ1 + RSB1)/(β + 1)]
        if (
            re1 is not None
            ...
            result["Rout1"] = parallel(
                re1,
                (rpi1 + result["RSB1"]) / (beta + 1),
            )
        ...
        # f2 due to interstage C2:
        # R22 = Rout1 + Ri2
        if result["Rout1"] is not None and result["Ri2"] is not None:
            result["R22"] = result["Rout1"] + result["Ri2"]

            if c2 is not None:
                result["f2"] = cutoff_frequency(result["R22"], c2)

        # f3 due to CE output C3:
        # R33 = RC2 + RL
        ...
        # Source resistance seen by CE stage 2 base:
        # RSB2 = Rout1 || RB2
        if result["Rout1"] is not None and rb2 is not None:
            result["RSB2"] = parallel(result["Rout1"], rb2)

        # fE due to CE emitter bypass:
        # REE = RE2 || [ RX2 + (rπ2 + RSB2)/(β + 1) ]
        if (
            re2 is not None
            and rx2 is not None
    ...
    def calculate_high(self, v, low):
        ...
        # CC stage 1 high-frequency:
        # Rπ1 = rπ1 || [RSB1 + (β + 1)REL1]
        if (
            rpi1 is not None
            and beta is not None
            and low.get("RSB1") is not None
            and low.get("REL1") is not None
        ):
            result["Rpi1"] = parallel(
                rpi1,
                low["RSB1"] + (beta + 1) * low["REL1"],
            )

        # Rµ1 = RSB1 || RXX1
        if low.get("RSB1") is not None and low.get("RXX1") is not None:
            result["Rmu1"] = parallel(low["RSB1"], low["RXX1"])
...
if __name__ == "__main__":
    ...
    w = CCCEMultistageFrequencyWidget()
    w.show()
    sys.exit(app.exec())